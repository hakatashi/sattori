...
| ゲームID | タイトル | 検証状況 |
| --- | --- | --- |
| `th06` | 東方紅魔郷 | 実リプレイ+ゲーム画面スクリーンショットで検証済み |
| `th07` | 東方妖々夢 | 同上 |
...
| `th11` | 東方地霊殿 | 同上 |
| `th12` | 東方星蓮船 | 同上 |
| `th125` | ダブルスポイラー | **未検証**（threplay移植のみ。テストデータ未入手） |
| `th128` | 妖精大戦争 | 実リプレイサンプルで検証済み |
| `th13` | 東方神霊廟 | 実リプレイ+スクリーンショットで検証済み |
| `th14` | 東方輝針城 | 同上 |
| `th143` | 弾幕アマノジャク | **未検証**（threplay移植のみ） |
| `th15` | 東方紺珠伝 | 実リプレイ+スクリーンショットで検証済み |
| `th16` | 東方天空璋 | 実リプレイサンプルで検証済み |
| `th165` | 秘封ナイトメアダイアリー | **未検証**（threplay移植のみ） |
| `th17` | 東方鬼形獣 | 実リプレイサンプルで検証済み |
| `th18` | 東方虹龍洞 | 同上 |
| `th20` | 東方錦上京 | プレイヤー名/日付/キャラ/難易度/ステージ/スコアは実リプレイ+スクリーンショットで検証済み。<br>**ステージ内訳（splits）は未対応**（下記参照） |
...
実際の `.rpy` は著作権物のためこのリポジトリには含めていません（`*.rpy` はリポジトリ全体で
`.gitignore` 対象）。ゴールデンテスト（`src/golden.test.ts`）は、開発機に兄弟リポジトリとして
`touhou-recorder/games/**` が存在する場合のみ実行され、存在しない環境（クローン直後やCI）では
自動的にスキップされます。
