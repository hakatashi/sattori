...
さらに、**重複フレーム率は「フレーム内容が完全に同一のまま複数回キャプチャされる」
現象しか検出できず、背景アニメーション等が常に変化し続けるタイトルでは「ゲーム全体の
進行が実時間に対して間延びする」タイプの処理落ちを見逃す**。th12（東方星蓮船）の
インスタンスタイプ実機検証（touhou-recorder reports/67）で、重複フレーム率0.6%という
「良好」に見える結果の裏で、リプレイの`frameCount`（60fps基準の理論尺）と実測プレイ
時間を比較すると理論尺超過+2.11%という深刻な処理落ちが隠れていた実例が見つかった。
**録画品質の評価には重複フレーム率だけでなく理論尺との比較も必ず併用すること**
（`worker/docs/titles/th12.md`）。

逆方向の誤りにも注意が必要で、**局所的に重複フレーム率が高くても、実際には処理落ちで
ないことがある**。th12のm7i.2xlarge実機検証（2026-09-01、
`docs/reports/2026-09-01-th12-2xlarge-instance-group-verification.md`）で、1面開始直後の
低モーション演出区間（背景アニメーションのみで動きが少ない）がたまたま録画開始
15〜45秒のスポット計測窓に重なり、重複フレーム率が12.7%（他インスタンスは0.1〜0.6%）
まで跳ね上がった事例が見つかった。秒単位で生存フレーム数を再集計すると、特定の瞬間に
重複が集中する「処理落ち」特有のパターンではなく、1〜2フレーム単位の軽い重複が全体に
薄く分布しているだけと判明し（プレイ本編全体では50%未満に落ち込む秒は0秒）、目視でも
処理落ちは確認できなかった。**単発の高い重複フレーム率だけで「処理落ちしている」と
断定せず、秒単位の生存フレーム再解析（`mpdecimate,showinfo`）と目視確認を併用すること**。
...
### GPUワーカーのWineクラッシュは検知できるが、AZ差の原因は未特定(Issue #267)

録画中にゲーム本体がWine上でクラッシュすると、フリーズした画面を画面静止検知が誤って
「リプレイ終了」と判定し、途中で切れた動画が`done`として配信されていた。現在は
`wine.log`に現れる`Unhandled`を検知して`classification="crashed"`とし、破棄して
リトライする（[`decisions/0054`](decisions/0054-detect-wine-crash-via-wine-log-not-duration-ratio.md)）。
**理論尺比（録画時間÷リプレイ推定時間）による異常検知は使っていない** ——
デシンクと原理的に区別できず、回復不能なデシンクに対して無駄なリトライを消費し、
価値のある途中までの動画をユーザーへ届けられなくなるため。
...
に実測で残してある（プロセス生存監視・プロセスstate監視・`winedbg`出現監視はいずれも
2つのクラッシュ形態の一方しか検知できない）。なお再現実験はNVIDIA GPUを持たない
マシンでCPU描画（Xvfb+llvmpipe）により行ったため、**GPU固有のクラッシュ形態が他に
ある可能性は排除できていない**。
...
### タイトル資産キャッシュは旧世代が最大6時間残る(Issue #104)

自宅ワーカーの `HOME_WORKER_TITLE_ASSETS_CACHE_DIR`（`worker/README.md` §8、
[`decisions/0040`](decisions/0040-home-worker-title-assets-cache.md)）は、単体テストに加えて
実機（本番のタイトル資産バケット・本番ワーカーイメージ、root権限で動くコンテナからの
bind mount越しの読み書き）でも動作確認済み（初回ダウンロード→展開→2回目以降のキャッシュ
再利用まで一通り確認、th06で約89秒→約1秒に短縮）。ただし**自宅ワーカーデーモン自身の
`docker run`経由でのマウント・環境変数受け渡しを、実際のジョブ(claim〜録画〜完了通知)
を通して確認したわけではない**(この検証は擬似S3クライアントを直接呼び出す形で行った)。
また、タイトル資産の更新（`upload-title-assets` skill）後も、削除を経過時間ベースの
保守的な判断にしているため**旧世代のディレクトリが最大6時間残り、その間はディスクを
二重に消費する**（世代あたり数百MB〜1GB程度。詳細は`decisions/0040`「決定」節）。
