...
`worker/README.md`「thprac によるデシンク対策」）。それでも**「プレイ時にthpracを
入れてください」という案内は消してはいけない**。両者が効く場所が違うためである。

デシンクの主因である**ANM再利用バグ（AnmVMのリセット漏れによる未初期化メモリ参照）は
「プレイ中」にランダムに発火する**。thpracはこれを常時修正しており
（`thprac_th20.cpp` の `THGuiCreate()` が `AnmVM_Reset_hook` を無条件で仕掛ける。
再生中限定ではない）、**プレイ時にthpracが入っていればそもそも汚染が起きない**。

逆に、**thprac無しでプレイした回にこのバグが発火してしまうと、保存されたリプレイ
ファイル自体が「再現不能な状態を通過した記録」になる**。リプレイは入力列と乱数種しか
持たず再生時に再シミュレーションする方式なので、記録時にだけ存在した未初期化メモリの
残骸は再生側からは復元しようがない。**これは再生側の対処（サーバー側のthpracも、
thprac自身のリプレイ修復ツール `TH_REPFIX` によるタイマーオフセット調整も）では
直せない**。thprac自身のUIテキストにも同じことが明記されている
（`thprac/src/thprac/thprac_games_def.json` の `TH20_MAIN_STORY_FIXES_DESC`）:

> なお、致命的なANM再利用バグ（ランダムに発生する表示崩れバグ）により同期ズレが
> 発生したリプレイは修復できません。thpracを適用してプレイした場合、これらのバグは
> 発生せず正常にリプレイを保存できます。
