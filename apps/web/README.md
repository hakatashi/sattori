...
（Issue #202）。自宅ワーカーは動画サイズ・回線次第でここに数分かかるが、バイト数
ベースの進捗までは報告しない（悲観バジェットも持たない）ため、`useOverallProgress.ts`
は変換完了時点（99%キャップ）で全体進捗バーを足踏みさせ、`done`到達で100%へ切り替える。
EC2ワーカーは同リージョンS3で一瞬なので実質素通りするだけだが、ワーカー内で自宅/EC2を
分岐させない方針（`AGENTS.md`）のため遷移自体は常に共通。
