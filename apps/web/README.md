...
th20（Issue #87）の品質を担保する。UI上の要件は「自宅ワーカーが使えるとき、かつその
場合に限り選べる」「使えるなら th20 だけ既定オン」「使えないならグレーアウト」。
...
- 既定値は`defaultSlowMotionFor(game, available)`（`@sattori/shared`）が決める。
  ...
  追従させない。
- 送信する値は「チェック状態」ではなく`slowMotionAvailable && slowMotion`という
  導出値を使う。可否やタイトルが変わったときにstateが取り残されないようにするため。
