...
配信用変換後のS3アップロードは`uploading`という独立したstatusで可視化する
（Issue #202）。自宅ワーカーは動画サイズ・回線次第でここに数分かかるため、実バイト数
ベースの進捗を報告する（Issue #202フォローアップ）: `status`が`uploading`へ遷移する
際、転送予定バイト数（`GetJobResponse.uploadTotalBytes`、転送開始前に判明済み）を
同時に記録し、以降`progress`（このフェーズでは転送済みバイト数、単位がrecording/
convertingの秒数と違う点に注意）が10秒間隔程度で届く。`jobProgressBudget.ts`は
`uploadTotalBytes`が判明した時点でuploading自体の悲観バジェット（`MIN_UPLOAD_BYTES_PER_SECOND`、
自宅ワーカーの実測 `docs/reports/2026-09-05-home-worker-upload-bandwidth.md` に基づく）を
全体バジェットへ積み増し、実際の転送比率（ratio）に応じて全体進捗バーが99%キャップの
先まで伸びるようにする（変換フェーズの実進捗率按分と同じ設計）。`uploadTotalBytes`が
未確定（旧ジョブ・最初の報告が届く前）の間は、従来どおり変換完了時点（99%キャップ）で
足踏みする。EC2ワーカーは同リージョンS3で転送が一瞬なため、この仕組みがあっても
実質素通りするだけだが、ワーカー内で自宅/EC2を分岐させない方針（`AGENTS.md`）のため
遷移自体は常に共通。
...
録画・変換・アップロードフェーズの進捗表示（「○:○○ 経過」またはアップロード中の
「○MiB / ○MiB」）は、ポーリング（3秒間隔）でしか届かないサーバー値を実時間で補間して
滑らかに見せている。サーバー値からの**外挿**（先読みして追い越し、次のポーリングで
同期＝巻き戻る）ではなく、**サーバー値を上限とする内挿**にしてある。フェーズに入った
時点で表示値をサーバー値より`WORKER_PROGRESS_INTERVAL_SECONDS`（=10秒。ワーカーが
進捗を書き込む間隔）ぶんの速度で手前に置き、そこからフェーズの速度で進める。遅れが
開いた場合（タブが裏に回ってティックが間引かれた等）は同じ秒数で埋め切る速度まで
一時的に上げて追いつく。`launching`のように進捗が意味を持たないフェーズでは、レコードに
値が残っていても表示しない。

アップロードフェーズの速度（バイト/秒）は変換フェーズの倍速推定と同じ方式——実測データが
無い間は自宅ワーカーの実測に基づく既定値（`DEFAULT_UPLOAD_RATE_BYTES_PER_SECOND`）で
補間し、同一フェーズ内で2回以上ポーリング結果が届いたら実測速度（`MIN_UPLOAD_BYTES_PER_SECOND`〜
`MAX_UPLOAD_RATE_BYTES_PER_SECOND`にクランプ）へ切り替える。
