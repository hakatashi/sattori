...
| --- | --- | --- |
| th06/06c/07/08/09/10 | `DEFAULT_CANDIDATE_INSTANCE_TYPES` | `c7i.xlarge` / `c7a.xlarge` / `c7i-flex.xlarge` / `m7i.xlarge` |
...
自宅ワーカーへ絶対に来ないため「割り当て先に応じて有効/無効が変わる」性質を
持たず、`WorkerEnvOptions`を経由する必要が無い。
