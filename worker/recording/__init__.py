...
th8_06.rpy は成功率20%)で発生することが分かった:
  ...
  - fps暴走: リプレイ再生中に内部fpsが数百〜数千に跳ね上がり、リプレイが
    ...
    ver1.00d への更新で事実上解消し(reports/23)、検知ロジック(MODログの
    GetDeviceState呼び出し頻度を閾値判定)は稀な残存ケースへの備えとして
    残していたが、実際に本番で発火した2件(th08の会話イベント、th09の
    ステージ間演出)がいずれも偽陽性で、真陽性の実績が皆無だったため
    2026-09-08に削除した([`decisions/0043`](../../docs/decisions/0043-remove-fps-runaway-detection.md)、
    同様の理由で先に削除した stutter probe の[`decisions/0038`](../../docs/decisions/0038-remove-stutter-early-detection.md)と同型の判断)。
  - 処理落ち: 録画開始直後の外乱でx11grabのフレームタイミングが崩れ、
    以降ずっと重複フレームで埋まる(reports/12・13由来、th08でも発生を確認)。

処理落ちの自動リトライは th08 固有の対策として作られたものだが、
th07 の開発初期(reports/12・13)にも観測された現象であり、リトライという
保険自体を両タイトル共通の仕組みにしておいて損はないため、本モジュールでは
th07 に対しても同じ検知・リトライ経路を適用する。既定のリトライ回数は3回
(ユーザー判断。th08固有の不安定性はver1.00d更新+音声分離修正でおおむね
解消された前提のため、旧検証時に推奨されていた8〜15回のような大きな値は
採用しない)。
