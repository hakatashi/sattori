...
reports/50 の検証はローカル(Ryzen 7 5700X)で行ったものだが、**AWS 実機(EC2 Fleet)でも
アタッチが成立することを本番ジョブ `439c14f3-3e6a-489e-817f-e00e039f841c` のワーカーログで
確認済み**。**アタッチに失敗しても録画自体は成功してしまう**(=デシンクが黙って再発する)
設計なので、th20 のジョブを調査する際はワーカーログに `thprac アタッチ完了` が出ているか、
