...
- **アタッチの予算は小さく保つこと**。アタッチは録画(ffmpeg)開始より前に行われるため、
...
- **thprac の出力と終了コードは必ずワーカーログへ流す**(`thprac出力:` / `thprac 終了
  (exit=...)`)。上記の失敗を調査したとき、旧実装が `DEVNULL` へ捨てていたせいで
  thprac 自身が何を言っていたのか事後に分からなかった(Issue #110)。
...
ジョブレコードが `workerKind: "home"`。以前ここに「AWS 実機で確認済み」と書いてあったのは
このジョブの取り違えだった)。reports/50 の検証もローカル(Ryzen 7 5700X)で行ったもの。

**アタッチに失敗しても録画自体は成功してしまう**(=デシンクが黙って再発する)設計なので、
...
## フェーズ開始時に進捗を戻す(`update_status(reset_progress=True)`、Issue #108)

`progress` は「**現在のフェーズ内**で処理が完了した時間(秒)」であって、フェーズを
跨いで意味を持たない。したがってフェーズを開始する書き込み(`recording` への遷移・
`converting` への遷移・チェックポイントからの変換再開)では、`status` と**同じ1回の
更新で** `progress` を0へ戻す。

分けて書くと、その隙間だけ「変換中なのに録画フェーズ末尾の進捗(＝リプレイ全長)」
「録画中なのに前の試行の進捗」というレコードがユーザーに見える。ジョブページの
経過時間表示は**巻き戻らないこと**を保証する作りになっている
(`apps/web/src/hooks/useEstimatedProgress.ts`)ため、この一瞬の値を掴むと以降の
進捗が表示に反映されず、そのフェーズの間ずっと止まって見える。窓を狭めるのではなく
原子的な更新にして窓自体を無くしてある。
