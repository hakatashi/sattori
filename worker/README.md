...
この性質は合成した検証用素材(`testsrc2`)で両方向を実測して確かめてある
(2026-08-11、`descale_to_normal_speed()` + `measure_duplicate_rate()`):

| 素材 | 変換前 | 変換後 | 判定 |
| --- | --- | --- | --- |
| 30fps相当を60fpsで録画(＝目標fpsを維持できた低速録画) | 49.9% | **0.0%** | 通過 |
| 15fps相当を60fpsで録画(＝本当に処理落ちした低速録画) | - | **50.0%** | 閾値30%超で正しくリトライ |

尺も 40.0 秒 → 20.03 秒と半分になり、音声のサンプルレートも 48000Hz へ戻ることを
確認済み。**「低速録画では重複フレーム率が理論上50%になるので閾値を緩める」という
対応は要らない**(touhou-recorder reports/47 の初版ではこれが未解決の課題として
残っていたが、変換を先に済ませることで根本から回避できる)。
...
(成果物: `worker/runs/th20-slowmo-th20_01/`)。

| 項目 | 実測 | 備考 |
| --- | --- | --- |
| 総録画時間(実時間) | 3366秒(56.1分) | touhou-recorder reports/47 の 3364.9秒とほぼ一致 |
| 終了検知 | 画面静止検知で正常終了 | 除外した2矩形があっても誤検知せず、本編中も正しく機能した |
| 等倍変換後の尺 | 1682.8秒(28.0分) | 実時間のちょうど1/2 |
| 重複フレーム率(変換後) | **0.3%** | 既定閾値30%をそのまま使って通過 |
| 出力 | 1280x960 / 60fps / 音声48000Hz | 音声は mean -11.9 dB(無音ではない) |

パイプラインの各所が意図どおり動いたことも個別に確認した:

- MODのキーシーケンス完了が開始から**29秒**(等倍基準の約14秒のほぼ2倍)。
  `ScaledSleep`と、それを待つ側のタイムアウトの両方がスケールしている。
- 処理落ちの早期検知(`probe_stutter`)は5回とも`dup_fraction=0.00`で**誤発火なし**。
  スケール未適用だとここが誤発火する(reports/47)。
