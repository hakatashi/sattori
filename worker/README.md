...
- **タイムアウト打ち切り(検知方式がリプレイ終了検知ではなく録画時間の上限)も、原因の
  fps低下自体を是正する手段は無い**(Issue #161)。`JobRecord.timedOut`に記録して
  デシンクと同じ警告表示に乗せ、ワーカーのログにも`WARNING:`を残すが、自動リトライは
  しない。fps低下(自宅ワーカーで観測、原因未特定)が起きたジョブは録画全体が遅く進むため、
  上限まで回してもリプレイ終端に届かず終盤が欠けたまま打ち切られる。
