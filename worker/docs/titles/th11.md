...
- **Pause Menu画面の選択カーソル明滅を静止判定から除外する**: 画面静止検知のみに
  ...
  `recording.vision.build_still_mask()`/`mad_masked()`でMAD計算から除外する
  (reports/38)。
- **クロップ座標は起動中の再配置を避けて確定する必要がある**: th11はopenboxの初期配置で
  ...
  にまとめてある（`recording.pipeline.attempt_recording()`）。
...
- **自動リトライ・音声/映像の別プロセス録画**はth06/07/08と共通の実装
  (`recording/`パッケージ)をそのまま使う。th11のMODも`fps_monitor.cpp`を
  組み込んでおり、実機検証では56.6〜60.2Hzで安定していた(fps暴走の兆候なし、
  reports/35)。処理落ちの早期検知(stutter probe)・fps暴走検知はいずれも
  真陽性の実績が無いまま正常なリプレイを誤検知しうることが判明したため削除済み
