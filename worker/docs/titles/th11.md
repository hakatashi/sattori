...
- **fps暴走の検知・自動リトライ・音声/映像の別プロセス録画**は
  th06/07/08と共通の実装(`recording/`パッケージ)をそのまま使う。th11のMODも
  `fps_monitor.cpp`を組み込んでおり、実機検証では56.6〜60.2Hzで安定していた
  (fps暴走の兆候なし、reports/35)。処理落ちの早期検知(stutter probe)は真陽性の実績が
  無く正常なリプレイも誤検知しうるため削除済み
