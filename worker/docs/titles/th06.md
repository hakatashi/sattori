...
- **wined3dの白画面ハング回避にVsyncPatchが必須**: th06はWine/最新Windows環境で
  ...
  `("vpatch_th06.dll",)`を指定すると、`recording.instance.build_injector_cmd()`がhook_dll
...
- **fps暴走の兆候は見られなかった**が、`mods/th06_replay_autoplay/`は
  `fps_monitor.cpp`を組み込んでいないため元々fps暴走検知は実質的に発火しなかった
  (th07と同様)。なお、fps暴走検知自体が真陽性の実績皆無・偽陽性のみだったため
  他タイトルとも共通で削除済み
  ([`decisions/0043`](../../../docs/decisions/0043-remove-fps-runaway-detection.md))。
  自動リトライ・音声/映像の別プロセス録画は th07/th08と共通の実装をそのまま使う。
  処理落ちの早期検知(stutter probe)は真陽性の実績が無く正常なリプレイも
  誤検知しうるため削除済み
...
- **実行ファイル名は元の`東方紅魔郷.exe`のまま使う(th07/th08のような`th{N}.exe`への
  ...
  `process_name`の明示オーバーライドを追加した(`recording/config.py`。未指定時は
...
（`recording.instance.prepare_instance()` の rsync で自動コピーされるため）。
