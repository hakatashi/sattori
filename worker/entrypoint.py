...
EC2 Fleet インスタンスの UserData から `docker run` で起動される。処理の流れ:
  ...
  2. (通常経路) S3からリプレイをダウンロード → recording状態 → record_th07.pyで録画
     (ProgressReporterが録画中のスクリーンショット/進捗をS3・DynamoDBへ反映する)
...
バックグラウンドで InterruptionWatcher が Spot中断通知/リバランス推奨を監視し、
検知次第 taskToken 経由で Step Functions に早期失敗通知する(60分のタイムアウトを
待たずに新インスタンスでのリトライを開始させるため)。録画/変換処理自体はそのまま
続行する(中断が実際に発効するかは分からないため、できるところまで進める)。
...
def on_interruption(reason, detail):
    """Spot中断/リバランス推奨を検知した際のコールバック。録画/変換処理自体は
    止めず、Step Functions側だけ早期に失敗させて新インスタンスでのリトライを促す。

    注: 通知後もこのインスタンスの処理は継続するため、実際には中断が発効せず
    (例えばリバランス推奨のみで実回収は起きない場合)そのまま処理が完了する
    ケースもあり得る。その場合でも taskToken は既に失敗として消費済みのため
    再度成功通知はしない(Step Functions側は別インスタンスで並行リトライ中)。
    HandleFailure Lambda がこの通知を受けてこのインスタンスを速やかに
    terminate するため、二重完了の実害は限定的。
    """
...
def convert_and_upload(s3):
    ...
    upscale_to_720p(OUTPUT_VIDEO, OUTPUT_VIDEO_720P, on_progress=on_convert_progress)
