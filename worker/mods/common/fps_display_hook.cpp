...
static ULONG_PTR g_targetRva = 0;
...
static BOOL WINAPI MyQueryPerformanceCounter(LARGE_INTEGER* lpPerformanceCount) {
    ...
    if (g_scale != 1.0 && rva == g_targetRva) {
        if (!g_epochSet) {
            g_epoch = real;
            g_epochSet = true;
        }
...
// timeGetTime版(th09)。QueryPerformanceCounter版とは独立した状態・IATスロットを使う。
...
static ULONG_PTR g_targetRvaTgt = 0;
...
static DWORD WINAPI MyTimeGetTime() {
    ...
    if (g_scaleTgt != 1.0 && rva == g_targetRvaTgt) {
        if (!g_epochSetTgt) {
            g_epochTgt = real;
            g_epochSetTgt = true;
        }
...
bool InstallFpsDisplayCorrectionHookTimeGetTime(ULONG_PTR targetRva) {
    ...
    Log("InstallFpsDisplayCorrectionHookTimeGetTime: IAT hook %s (scale=%.4f, targetRva=0x%08lx)",
        ok ? "OK" : "FAILED", g_scaleTgt, (unsigned long)g_targetRvaTgt);
    return ok;
}

bool InstallFpsDisplayCorrectionHook(ULONG_PTR targetRva) {
    g_targetRva = targetRva;
    double targetHz = 60.0;
    const char* env = getenv("FPS_LIMIT_TARGET_HZ");
    if (env) {
        double hz = atof(env);
    ...
    Log("InstallFpsDisplayCorrectionHook: IAT hook %s (scale=%.4f, targetRva=0x%08lx)",
        ok ? "OK" : "FAILED", g_scale, (unsigned long)g_targetRva);
    return ok;
}
