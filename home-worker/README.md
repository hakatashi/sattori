...
| `HOME_WORKER_SUPPORTED_GAMES` | `SUPPORTED_GAME_IDS`からGPU描画必須タイトルを除いた全部（現状th06/06c/07/08/09/10/11/12/20） | 引き受けるタイトル。自宅マシンの都合で一部だけ受け持ちたい場合に上書きする。**GPU描画必須タイトル（th06nc、Issue #241）はGPU非搭載の自宅マシンでは録画できないため指定できない**（指定すると`ConfigError`で起動失敗する） |
