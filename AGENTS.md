...
**ドキュメントに何かを書き足す前に
[`docs/documentation-guidelines.md`](docs/documentation-guidelines.md) を読むこと。**
ドキュメントを性質で4分類し、書き換えるファイルには追記せず、追記型の情報は
1件1ファイルで増やすことで肥大化を構造的に止めるのが方針（Issue #112）。判断フローの
要約は次の通り。

- **現在の仕様が変わった** → 該当パッケージの `README.md` を**書き換える**（追記しない）
- **なぜそうしたかの根拠・踏んだ地雷** → `docs/decisions/` に**新規ファイル**（軽量ADR、
  連番 `NNNN-kebab-case-title.md`）
- **実機検証・実測をした** → `docs/reports/` に**新規ファイル**（`YYYY-MM-DD-title.md`、不変）
- **手順が増えた** → `docs/runbooks/` か Skill

移設・分割の際は README と決定記録の間に必ず相互リンクを張ること。リンクが無いと、
エージェントは決定記録の存在に気づけないまま README の仕様だけを読んで推測で変更する
（§3 が最も警戒している事態）。なお**このファイル自体は毎セッション常時ロードされる**ため、
追記型の情報（既知の制約・検証結果）を置いてはならない。上限は150行 / 12 KB。

## 8. Issue・GitHub Projects の運用
