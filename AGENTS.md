...
[EC2 Fleet ワーカー(Spot, Docker: Wine+Xvfb+ffmpeg+Python)]
   ...
   (Spot中断/リバランス推奨の2分前通知をIMDS経由で監視し、検知次第taskTokenで
   早期に失敗通知してリトライを早める)
        ▼
...
- **録画ジョブは Step Functions（Standard）でオーケストレーション**（Issue #11）。
  ...
  最大3回まで自動リトライする。ワーカーはIMDS経由でSpot中断通知/リバランス推奨
  （いずれも2分前通知）を監視し、検知次第60分タイムアウトを待たずに早期失敗通知する。
...
- `entrypoint.py`: ジョブの全体制御。DynamoDBの`outputPath`(生動画チェックポイント)
  ...
  （Spot中断/リバランス推奨をIMDS経由で監視）と`ProgressReporter`（録画中の
  進捗スクリーンショット/進捗率をS3・DynamoDBへ反映）を動かす。taskToken経由で
