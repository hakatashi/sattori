...
- **【要調査】th08（および恐らくth07）のリプレイ終了検知テンプレート画像
  （`worker/assets/replay_end_templates/th08.png`・`th07.png`）が、実際の録画に
  写り込むウィンドウのタイトルバー分だけ縦にずれており、テンプレート照合が機能しない
  可能性がある（touhou-recorder reports/43で発見。同リポジトリの検証環境の画像で
  確認されたもので、このリポジトリ自身の画像が同じ問題を抱えているかは未確認）。
  機能しない場合、th08/th07のジョブは`TIMEOUT_SEC`（`worker/recording_common.py`）
  到達まで録画し続け、動画末尾に静止画面が付く形で完了する。インスタンスタイプ
  選定とは無関係の録画パイプライン側の問題であり、修正されるまで前述の候補
  インスタンスタイプの合否判定には影響しない。
