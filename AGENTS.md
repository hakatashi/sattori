...
- 自宅サーバーワーカー（Issue #49）は実装済み。開発マシン（16コア）でフル尺2本の
  2並列実機録画を完走することは確認済み（th07 Hard クリア37.2分 + th08 Normal
  クリア35.0分、いずれもリトライなし。詳細は`home-worker/README.md`「実機検証の記録」）。
  ただし**claim競合・claim取り消し・オファー経由のE2E（AWSを通した割り当て）は未検証**
  で、これらはPRのデプロイ後に確認する必要がある。
- **2並列時、録画開始から5〜10分の区間だけ描画fpsが23〜28fpsまで落ちる**ことを実測した
  （ゲーム内fps表示。以降は60.00fpsで安定）。総録画時間はリプレイ長とほぼ一致するため
  ゲームロジックは60Hzを維持しており、th11で問題になった「実時間の伸長」ではなく
  **描画のコマ落ち**である。原因は未特定で、単独録画のベースラインも未取得のため
  「2並列の競合」と断定はできない（資産展開直後のI/O、wined3dのステート構築など
  起動直後の一過性要因の可能性がある）。「自宅マシンは4並列まで処理落ちなく録画可能」
  という当初の想定（Issue #48）は、少なくとも録画前半については再検証が必要。
- **重複フレーム率の自動チェック（`recording_common.measure_duplicate_rate`）は
  録画開始15〜45秒の30秒スポットしか見ていない**ため、上記のように数分後に現れる
  コマ落ちを取りこぼす（上記のth07は6.9%で閾値を通過したが、5分時点では28%だった）。
  この区間の値をタイトル間・環境間で比較する際は「全編の代表値ではない」ことに注意すること。
